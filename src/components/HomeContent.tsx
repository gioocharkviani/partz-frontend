...
export default function HomeContent({
  ...
  const avgRating = shops.length
    ? (
        shops.reduce(
          (s: number, sh: any) => s + Number(sh.rating_avg || 0),
          0,
        ) / shops.length
      ).toFixed(1)
    : "—";
  const completedOrders = shops.reduce(
    (s: number, sh: any) => s + Number(sh.completed_orders_count || 0),
    0,
  );
    ...
    <>
      ...
      <section className="py-14 bg-screen">
        <div className="max-w-375 mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-11">
            <span className="inline-block px-3.5 py-1 bg-white/70 text-teal-dark font-bold text-xs tracking-widest uppercase rounded-full mb-3 border border-forest-green/15">
              {t("home.howItWorks")}
            </span>
            <h2 className="text-2xl font-black text-forest-green">
              {t("home.howTitle")}
            </h2>
            <p className="text-forest-green/70 mt-2 max-w-xl mx-auto text-sm">
              {t("home.howDesc")}
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {steps.map((item, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-7 shadow-lg text-center"
              >
                <div
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-4 mt-8">
                ...
                <div className="bg-white border border-teal-border rounded-2xl p-6 card-shadow">
                  ...
                  <div className="text-sm font-semibold text-dark">
                    {t("home.ordersCompleted")}
                  </div>
                  <div className="text-xs text-muted mt-0.5">
                    {t("home.paymentReleased")}
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-16 bg-neutral-wash relative overflow-hidden">
        <div className="relative max-w-3xl mx-auto px-4 text-center">
          <h2 className="text-3xl lg:text-4xl font-black text-forest-green mb-4">
            {t("home.ctaTitle")}
          </h2>
          <p className="text-forest-green/70 mb-8 text-lg">
            {t("home.ctaDesc")}
          </p>
